...
class Python:
    ...
    def __post_init__(self):
        self.code = self.code.replace(ADD, '+').replace(MUL, '*')
op_to_mark = lambda score: re.sub('`[^`]*`', lambda m: m.group(0).replace('+', ADD).replace('*', MUL), score)
...
def _compile_取不出名(notes: str, unit, vel, key, hook: Callable[[Note], list[Note]] | None) -> list[Note]:
    ...
    key_to_pitch[0] = None
    propress_note_and_chord = []
    new_note = lambda i: Note(i.start * unit, i.lasting * unit, i.base_pitch, i.offset, i.vel if i.vel is not None else vel, i._hook, i.length * unit, scale=unit)
    for i in ret:
        if isinstance(i, Note):
            propress_note_and_chord.append(i._hook(new_note(i)) if i._hook else (hook(new_note(i)) if hook else [new_note(i)]))
        elif isinstance(i, Chord):
