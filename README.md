...
引用定义的其他乐谱，同样支持临时属性
### 音符
可选的\`<python_expr>\`+可选的零到多个.或零到多个'+可选的零到多个s或零到多个f+必选的0-7数字+可选的零到多个-+可选的零到多个_ <br>
如\`func\`''f2---__，每有一个'升高八度，每有一个.降低八度，每有一个s升高半音，每有一个f降低半音，每有一个-延长一拍时值，每有一个_延音一拍，数字为0时是休止符，前面的python表达式是对音符进行处理的函数。
### 和弦
```
[ <note> <note> ... ]
```
...
[ <note> <note> ... ]---__
```
中括号包裹一到多个音符，末尾是可选的延音线和延时值线，会把内部音符统一设置成相应的时值和音长
