...
def prepreprocess_include(file: str, include_path: list[str] = None):
    ...
    while True:
        new = re.sub(r'\{\%\s*include\s*([^{}]+)\s*\%\}|^\s*\%\s*include\s*([^\n]+)|\n\s*\%\s*include\s*([^\n]+)', lambda m: prepreprocess_include(search(literal_eval([i for i in m.groups() if i][0].strip())), include_path), code)
        if code == new:
            break
        code = new
    return code
...
def preprocess(file: str, include_path: list[str] = None):
    ...
    prepre_code = prepreprocess_macro(code)
    env = Environment(
    ...
    try:
        tmpl = env.from_string(prepre_code)
        return tmpl.render()
    except Exception as e:
        raise PreprocessError(e, prepre_code)
